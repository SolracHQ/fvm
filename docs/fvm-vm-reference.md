...
`cr` (context register) is a single shared 32-bit register not part of either
file. It holds the active address space context identifier. It is readable in
any mode but writable only from kernel mode. Writing it in user mode raises
interrupt 6.
...
bits 7:5  view
            ...
            111 = reserved
bit  4    reserved
bits 3:0  register index 0-15 (ignored for sp and cr)
```
...
enabled devices. RAM is placed first at physical address `0x00000000`, then
each additional device is placed immediately after the previous one. The result
is a linear physical layout fixed at startup and never modified at runtime.

RAM size is configured by the user at startup. It is not required to be 4 GB
or any fixed size.

The first bytes of RAM hold a discovery table so the kernel can learn what
devices are present and where. The table is written by the VM at startup.
...
All mapped regions must start and end on 4 kb page boundaries. Section sizes
in the binary format are therefore rounded up to the nearest 4 kb boundary by
the loader before mapping.
...
The stack region is always mapped regardless of RAM size. It maps the last
4 MB of physical RAM to the fixed virtual range `0xFFC00000..0xFFFFFFFF`. `sp`
in the kernel file is initialised to `0xFFFFFFFF` on VM creation.

If the end of `.data` would overlap `0xFFC00000` the loader returns an error.

### Fault info region

A reserved page in context 0 following the device table, accessible only from
kernel mode. The VM writes fault information here on every interrupt entry
...
- CR value at the time of the fault
...
returns `Err(FvmError::Interrupt(n))`. The bus propagates this to the execution
...
The interrupt vector table is stored inside `Vm`, not in bus memory. `LOAD`
and `STORE` cannot reach it.
...
only thing that changes on a mode switch. All instructions operate on
`files[active]`.

`SIE`, `DPL`, `TUR`, `TKR`, `MMAP`, `MUNMAP`, `MPROTECT`, and writes to `cr`
are privileged. Executing any of them when `active == 1` raises interrupt 6.
...
Reservation ranges within `16..255` for specific hardware device classes will
be defined when device support is implemented.

### Interrupt dispatch

`raise_interrupt(vm, index)` is the only entry point. It always switches to
and runs in the kernel file.

Dispatch sequence:

1. Write fault information to the fault info region.
2. If `pending_interrupt` is `Some(1)` and the incoming index is also `1`,
   this is a double fault: halt the VM immediately.
...
The interrupted `ip` pushed in step 6 is chosen by the caller before
...
Total frame size pushed onto the kernel stack: 73 bytes.

### IRET

`IRET` unwinds exactly one interrupt frame from the kernel stack:

1. Pop `came_from_user` (1 byte), `flags` (4 bytes), `ip` (4 bytes), and the
   16 general-purpose registers (64 bytes) from kernel `sp` in reverse push
   order.
2. If `came_from_user == 1`, restore the popped state into the user file and
   set `active = 1`. Otherwise restore into the kernel file and stay in kernel
   mode.
...
1. Build the physical layout: write the device discovery table into the first
   bytes of RAM.
2. Compute the virtual base address for each non-empty section by starting
   after the fixed regions (device table, fault info) and advancing by
   `ceil(section_size / 4096) * 4096` for each section in order.
3. Walk the relocation table. Each entry `(section: u8, offset: u32)`
   identifies a 4-byte slot within that section. Patch the slot by replacing
   the assembler-assumed address with the actual loaded address:
...
|----------|----------|--------|
| `SIE rb, label` | `op rw_enc imm32[4]` | `ivt[rw] = imm32` |
| `SIE rb, rw` | `op rw_enc rw_enc` | `ivt[rw] = rw` |
...
| `TUR rw, rw` | `op rw_enc rw_enc` | copy user register into kernel register |
| `TKR rw, rw` | `op rw_enc rw_enc` | copy kernel register into user register |
| `MMAP rw, rw, rw\|imm32` | `op rw_enc rw_enc rw_enc\|imm32` | map physical pages into current context |
| `MUNMAP rw, rw\|imm32` | `op rw_enc rw_enc\|imm32` | unmap virtual addresses from current context |
| `MPROTECT rw, rw\|imm32, rb` | `op rw_enc rw_enc\|imm32 rb_enc` | set permission bits on the page range |

Rules:

- `SIE`, `DPL`, `TUR`, `TKR`, `MMAP`, `MUNMAP`, and `MPROTECT` are
  privileged. Executing any of them in user mode raises interrupt 6.
- `IRET` with nothing on the kernel stack raises interrupt 6.
- `SIE` index and address operands must be `rb`.
...
- `TUR` and `TKR` operands must both be `rw`.
- `MMAP`, `MUNMAP`, and `MPROTECT` address operands must be `rw`. The size
  operand of `MMAP` also accepts `imm32`.
- `MPROTECT` permission operand must be `rb`.
- `MMAP` size is rounded up to the nearest 4 kb page boundary.