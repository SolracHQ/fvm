...
suite "initRom":
  ...
  test "sets ip to entryPoint":
    var vm = newVm().get()
    let obj = FvmObject(version: FvmVersion, entryPoint: 0x0010'u16,
                        code: @[Byte(ord(OpCode.Halt))])
    vm.initRom(obj).get()
...
suite "step - MOV / PUSH / POP":
  test "MOV r0, 42 sets r0":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0'u8, 0'u8, 42'u8,
  ...
  test "PUSH r0 then POP r1 copies value":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0'u8, 0'u8, 0xAB'u8,  # MOV r0, 0xAB
      Byte(ord(OpCode.Push)),      0'u8,                    # PUSH r0
      Byte(ord(OpCode.Pop)),       1'u8,                    # POP  r1
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "MOV r0, r1 copies register":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 1'u8, 0'u8, 0x55'u8,  # MOV r1, 0x55
      Byte(ord(OpCode.MovRegReg)), 0'u8, 1'u8,             # MOV r0, r1
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "MOV r0, 0x1234 stores 16-bit value":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0'u8, 0x12'u8, 0x34'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "MOV r0l writes low byte only leaving high byte intact":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0'u8, 0x12'u8, 0x34'u8,  # MOV r0, 0x1234
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xAB'u8,         # MOV r0l, 0xAB
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
...
suite "step - ADD":
  test "ADD r0, r1 full-width":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x05'u8,  # MOV r0, 5
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x03'u8,  # MOV r1, 3
      Byte(ord(OpCode.Add)),       0x00'u8, 0x01'u8,            # ADD r0, r1
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[0] == 8
  ...
  test "ADD carry on word overflow":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0xFF'u8, 0xFF'u8,  # MOV r0, 0xFFFF
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x01'u8,  # MOV r1, 1
      Byte(ord(OpCode.Add)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "ADD byte-lane carry on overflow":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xFF'u8,            # MOV r0l, 0xFF
      Byte(ord(OpCode.MovRegImm)), 0x81'u8, 0x01'u8,            # MOV r1l, 0x01
      Byte(ord(OpCode.Add)),       0x80'u8, 0x81'u8,            # ADD r0l, r1l
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "ADD sets zero flag":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x00'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x00'u8,
      Byte(ord(OpCode.Add)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
...
suite "step - SUB":
  test "SUB r0, r1 basic":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x0A'u8,  # MOV r0, 10
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x03'u8,  # MOV r1, 3
      Byte(ord(OpCode.Sub)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[0] == 7
  ...
  test "SUB borrow sets carry":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x03'u8,  # MOV r0, 3
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x05'u8,  # MOV r1, 5
      Byte(ord(OpCode.Sub)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "SUB equal values sets zero flag":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x07'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x07'u8,
      Byte(ord(OpCode.Sub)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
...
suite "step - AND/OR/XOR/NOT":
  test "AND r0, r1":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0xFF'u8,  # r0 = 0x00FF
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x0F'u8, 0x0F'u8,  # r1 = 0x0F0F
      Byte(ord(OpCode.And)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "OR r0, r1":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0xF0'u8, 0x00'u8,  # r0 = 0xF000
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x0F'u8,  # r1 = 0x000F
      Byte(ord(OpCode.Or)),        0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "XOR r0 with itself zeroes register":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x12'u8, 0x34'u8,
      Byte(ord(OpCode.Xor)),       0x00'u8, 0x00'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[0] == 0
  ...
  test "NOT r0":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0xFF'u8, 0x00'u8,  # r0 = 0xFF00
      Byte(ord(OpCode.Not)),       0x00'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "NOT byte-lane":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0x0F'u8,           # MOV r0l, 0x0F
      Byte(ord(OpCode.Not)),       0x80'u8,                     # NOT r0l
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
...
suite "step - CMP":
  test "CMP equal sets zero, clears carry":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x0A'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x0A'u8,
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.flags.zero
  ...
  test "CMP src > dst sets carry":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x03'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x05'u8,
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
...
suite "step - OUT":
  test "OUT byte sends single byte to port":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xAB'u8,            # MOV r0l, 0xAB
      Byte(ord(OpCode.Out)),       0x00'u8, 0x80'u8,             # OUT 0, r0l
      Byte(ord(OpCode.Halt)),
    ])
    vm.ports.registerPort(0, PortDevice(
      label: "capture",
      read: proc(): FvmResult[Byte] = Byte(0).ok,
  ...
  test "OUT word sends hi then lo byte":
    var captured: seq[Byte]
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x12'u8, 0x34'u8,   # MOV r0, 0x1234
      Byte(ord(OpCode.Out)),       0x00'u8, 0x00'u8,             # OUT 0, r0
      Byte(ord(OpCode.Halt)),
    ])
    vm.ports.registerPort(0, PortDevice(
      label: "capture",
...
suite "step - IN":
  test "IN byte lane reads from port":
    var vm = freshVm(@[
      Byte(ord(OpCode.In)), 0x80'u8, 0x00'u8,                   # IN r0l, 0
      Byte(ord(OpCode.Halt)),
    ])
    vm.ports.registerPort(0, PortDevice(
      label: "provide",
      read: proc(): FvmResult[Byte] = Byte(0xCD).ok,
      write: proc(v: Byte): FvmResult[void] = ok(),
    )).get()
    vm.run().get()
  ...
  test "IN word assembles hi then lo from port":
    var counter = 0
    var vm = freshVm(@[
      Byte(ord(OpCode.In)), 0x00'u8, 0x00'u8,                   # IN r0, 0
      Byte(ord(OpCode.Halt)),
    ])
    vm.ports.registerPort(0, PortDevice(
      label: "provide",
...
suite "step - JMP":
  test "JMP immediate skips instructions":
    # layout:
    #   [0] JMP 0x0007          3 bytes  -> jumps to offset 7
    #   [3] MOV r0, 1           4 bytes  -> skipped
    #   [7] HALT
    var vm = freshVm(@[
      Byte(ord(OpCode.Jmp)),       0x00'u8, 0x07'u8,
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[0] == 0
  ...
  test "JNZ taken (zero=false) skips fall-through":
    ...
    #   [7]  JNZ 0x000E      3 bytes  -> jumps to 14
    #   [10] MOV r2, 1       4 bytes  -> skipped
    #   [14] HALT
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Jnz)),       0x00'u8, 0x0E'u8,
      Byte(ord(OpCode.MovRegImm)), 0x02'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[2] == 0
  ...
  test "JNZ not taken (zero=true) continues":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Jnz)),       0x00'u8, 0x0B'u8,
      Byte(ord(OpCode.MovRegImm)), 0x02'u8, 0x00'u8, 0xFF'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "JZ taken (zero=true) skips fall-through":
    ...
    #   [3] JZ 0x000A        3 bytes  -> jumps to 10
    #   [6] MOV r1, 1        4 bytes  -> skipped
    #   [10] HALT
    var vm = freshVm(@[
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x00'u8,
      Byte(ord(OpCode.Jz)),        0x00'u8, 0x0A'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
    check vm.regs[1] == 0
  ...
  test "JZ not taken (zero=false) continues":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Cmp)),       0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Jz)),        0x00'u8, 0x0F'u8,
      Byte(ord(OpCode.MovRegImm)), 0x02'u8, 0x00'u8, 0xAB'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "JmpReg (indirect) jumps to address in register":
    # layout:
    #   [0]  MOV r0, 11      4 bytes  -> r0 = target address
    #   [4]  JmpReg r0       2 bytes  -> jump to r0=11
    #   [6]  MOV r1, 1       4 bytes  -> skipped (offsets 6-9)
    #   [10] HALT                     -> skipped
    #   [11] HALT
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x00'u8, 0x0B'u8,
      Byte(ord(OpCode.JmpReg)),    0x00'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0x01'u8,
      Byte(ord(OpCode.Halt)),
      Byte(ord(OpCode.Halt)),
    ])
...
suite "step - CALL / RET":
  test "CALL pushes return address and jumps, RET returns":
    # layout:
    #   [0]  CALL 0x0008     3 bytes  -> retAddr=3, jumps to 8
    #   [3]  MOV r1, 0xFF    4 bytes  -> executes after RET
    #   [7]  HALT
    #   [8]  MOV r0l, 0xAB   3 bytes  -> subroutine body
    #   [11] RET              1 byte   -> returns to 3
    var vm = freshVm(@[
      Byte(ord(OpCode.Call)),      0x00'u8, 0x08'u8,
      Byte(ord(OpCode.MovRegImm)), 0x01'u8, 0x00'u8, 0xFF'u8,
      Byte(ord(OpCode.Halt)),
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xAB'u8,
      Byte(ord(OpCode.Ret)),
    ])
    vm.run().get()
  ...
  test "nested CALL / RET restores correct return addresses":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.Call)),      0x00'u8, 0x04'u8,  # CALL outer (offset 4)
      Byte(ord(OpCode.Halt)),
      Byte(ord(OpCode.Call)),      0x00'u8, 0x0B'u8,  # CALL inner (offset 11)
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xBB'u8,  # MOV r0l, 0xBB
      Byte(ord(OpCode.Ret)),
      Byte(ord(OpCode.MovRegImm)), 0x80'u8, 0xAA'u8,  # MOV r0l, 0xAA
      Byte(ord(OpCode.Ret)),
    ])
    vm.run().get()
...
suite "step - SP register":
  test "MOV r0, sp reads stack pointer":
    ...
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegReg)), 0x00'u8, 0x40'u8,
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
  ...
  test "MOV sp, r0 writes stack pointer":
    var vm = freshVm(@[
      Byte(ord(OpCode.MovRegImm)), 0x00'u8, 0x12'u8, 0x34'u8,  # MOV r0, 0x1234
      Byte(ord(OpCode.MovRegReg)), 0x40'u8, 0x00'u8,             # MOV sp, r0
      Byte(ord(OpCode.Halt)),
    ])
    vm.run().get()
