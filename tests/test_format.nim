...
suite "FvmObject serialize / deserialize":
  test "round-trip: serialize then deserialize":
    let original = FvmObject(
      version: FvmVersion, entryPoint: 0x0010'u16,
      code: @[0x00'u8, 0x01'u8],
    )
  ...
  test "header declares more code bytes than present returns error":
    var bytes = FvmObject(version: FvmVersion, entryPoint: 0'u16,
                          code: @[0x00'u8]).serialize()
